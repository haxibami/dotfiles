...
setdefault () {
  menu=$(pamixer --list-$1s | grep -e $2 -e $1 -e "Headset" -e "WebCam" -e "Microphone" | grep -v "monitor" | awk -F\" '{print $6}' | wofi -i -b -p $1-list --dmenu --width 1000 --lines=5 --style=${HOME}/.config/wofi/powermenu.css --conf=${HOME}/.config/wofi/powermenu.conf) &&
